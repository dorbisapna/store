...
$stmt=$conn->prepare("SELECT name,new_price,old_price,image FROM dailyshop WHERE category=?");
$stmt->bind_param("s",$clothes);
$stmt->bind_result($w_name,$w_new_price,$w_old_price,$w_image);
$stmt->execute();
while($stmt->fetch())
{
  $women[]=array("name"=>$w_name,"new_price"=>$w_new_price,"old_price"=>$w_old_price,"image"=>$w_image);
}
...
$stmt=$conn->prepare("SELECT name,new_price,old_price,image FROM dailyshop WHERE category=?");
$stmt->bind_param("s",$clothes);
$stmt->bind_result($m_name,$m_new_price,$m_old_price,$m_image);
$stmt->execute();
while($stmt->fetch())
{
$man[]=array("name"=>$m_name,"new_price"=>$m_new_price,"old_price"=>$m_old_price,"image"=>$m_image);
}

$sports = array();
$games = "sports";
$stmt=$conn->prepare("SELECT name,new_price,old_price,image FROM dailyshop WHERE category=?");
$stmt->bind_param("s",$games);
$stmt->bind_result($m1_name,$m1_new_price,$m1_old_price,$m1_image);
$stmt->execute();
while($stmt->fetch())
{
$sports[]=array("name"=>$m1_name,"new_price"=>$m1_new_price,"old_price"=>$m1_old_price,"image"=>$m1_image);
}

$electronic_item = array();
$electronics = "electronics";
$stmt=$conn->prepare("SELECT name,new_price,old_price,image FROM dailyshop WHERE category=?");
$stmt->bind_param("s",$electronics);
$stmt->bind_result($m2_name,$m2_new_price,$m2_old_price,$m2_image);
$stmt->execute();
while($stmt->fetch())
{
$electronic_item[]=array("name"=>$m2_name,"new_price"=>$m2_new_price,"old_price"=>$m2_old_price,"image"=>$m2_image);
}
                 ...
                 <ul class="nav nav-tabs aa-products-tab">
                  ...
                  <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="electronics">
                       <ul class="aa-product-catg">
                        <!-- start single product item -->
                        <?php foreach($electronic_item as $key => $value): ?>
                        <li>
                          <figure>
